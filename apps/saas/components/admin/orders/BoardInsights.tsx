...
import { ShoppingBag, Clock, TrendingUp, Package, Truck, CheckCircle2 } from 'lucide-react'
...
const STATUS_CONFIG = [
  { status: 'pending',   label: 'Pendientes',  icon: Clock,        color: 'bg-amber-400',  textColor: 'text-amber-700',  bgColor: 'bg-amber-50' },
  { status: 'confirmed', label: 'Confirmados', icon: Package,      color: 'bg-blue-500',   textColor: 'text-blue-700',   bgColor: 'bg-blue-50' },
  { status: 'preparing', label: 'Preparando',  icon: Package,      color: 'bg-orange-400', textColor: 'text-orange-700', bgColor: 'bg-orange-50' },
  { status: 'ready',     label: 'Listos',      icon: CheckCircle2, color: 'bg-emerald-500',textColor: 'text-emerald-700',bgColor: 'bg-emerald-50' },
  { status: 'en_ruta',   label: 'En Ruta',     icon: Truck,        color: 'bg-sky-500',    textColor: 'text-sky-700',    bgColor: 'bg-sky-50' },
]

export default function BoardInsights({ orders }: Props) {
  const totalOrders = orders.length
  const activeOrders = orders.filter(o => ['pending', 'confirmed', 'preparing', 'ready', 'en_ruta', 'arrived'].includes(o.status))
