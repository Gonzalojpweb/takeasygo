...
export default function EditTenantForm({ tenant, adminEmail: initialAdminEmail }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    featuresPosEnabled: tenant.features?.posEnabled ?? false,
    ...
    try {
      const res = await fetch(`/api/superadmin/tenants/${tenant._id}`, {
        ...
        body: JSON.stringify({
          ...
          features: { reservations: form.featuresReservations, crm: { enabled: form.featuresCrmEnabled }, tgoGrowthPushEnabled: form.featuresTgoGrowthPushEnabled, cashPaymentEnabledBySuperadmin: form.featuresCashPaymentEnabledBySuperadmin, posEnabled: form.featuresPosEnabled },
        ...
        <CardContent className="p-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between p-5 bg-muted/30 border-2 border-border/40 rounded-[2rem] h-[58px] max-w-sm">
                <div className="flex items-center gap-3">
                  <div className={cn("w-2 h-2 rounded-full", form.featuresPosEnabled ? "bg-primary shadow-[0_0_8px_rgba(var(--primary),0.5)]" : "bg-muted-foreground/30")} />
                  <span className="text-[10px] uppercase font-black tracking-widest text-muted-foreground">POS Bundle (Sync Layer)</span>
                </div>
                <button type="button"
                  onClick={() => setForm(p => ({ ...p, featuresPosEnabled: !p.featuresPosEnabled }))}
                  className={cn(
                    "w-12 h-6 rounded-full transition-all relative flex items-center",
                    form.featuresPosEnabled ? 'bg-primary' : 'bg-muted-foreground/20'
                  )}>
                  <div className={cn(
                    "w-4 h-4 rounded-full bg-white shadow-sm transition-all absolute",
                    form.featuresPosEnabled ? 'left-[26px]' : 'left-1'
                  )} />
                </button>
              </div>
