...
export default async function SuperAdminAnalyticsPage() {
  ...
  return (
    <div>
      <h1 className="text-white text-2xl font-bold mb-6">Analytics Global</h1>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8 lg:grid-cols-4">
        ...
        <Card className="bg-zinc-800 border-zinc-700">
          ...
          <CardContent>
            ...
            <p className="text-zinc-500 text-xs mt-1">{lastMonth.count} el mes pasado</p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card className="bg-zinc-800 border-zinc-700">
          <CardHeader>
            <CardTitle className="text-white text-base">Tenants más activos</CardTitle>
          </CardHeader>
          <CardContent>
            {topTenants.length === 0 ? (
                  ...
                  return (
                    <div key={item._id} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white text-sm">{tenant?.name || 'Desconocido'}</p>
                          <p className="text-zinc-500 text-xs font-mono">{tenant?.slug}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-white text-sm font-bold">{item.totalOrders} pedidos</p>
                        <p className="text-zinc-500 text-xs">${item.totalRevenue.toLocaleString('es-AR')}</p>
                      </div>
                    </div>
                  )
                })}
        ...
        <Card className="bg-zinc-800 border-zinc-700">
          <CardHeader>
            <CardTitle className="text-white text-base">Últimos pedidos globales</CardTitle>
          </CardHeader>
          <CardContent>
            {recentOrders.length === 0 ? (
              ...
              <div className="space-y-3">
                {recentOrders.map((order: any) => (
                  <div key={order._id} className="flex items-center justify-between py-1 border-b border-zinc-700 last:border-0">
                    <div>
