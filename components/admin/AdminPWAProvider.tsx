...
function isIOSDevice(): boolean {
  ...
  return /iphone|ipad|ipod/.test(ua) && !/chrome/.test(ua)
}

export default function AdminPWAProvider({ primaryColor, bgColor, textColor, manifestUrl }: Props) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm" style={{ color: textColor }}>
              {ios ? 'Agregar al inicio' : 'Instalar app de admin'}
            </p>
            {ios ? (
              ...
              <p className="text-xs mt-0.5" style={{ color: textColor, opacity: 0.55 }}>
                Recibí notificaciones de pedidos al instante
              </p>
            )}
          </div>
