...
export default function BusinessCompaniesClient({ companies: initial, tenantSlug }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/${tenantSlug}/business/companies`, {
        method: 'POST',
      ...
      setShowCreate(false)
      setForm({ companyName: '', companyTaxId: '', companyAdminEmail: '', paymentMode: 'cash_mp', paymentTerms: '', employeeEmailsCsv: '', notes: '' })
  ...
  async function handleToggleStatus(company: Company) {
    ...
    try {
      const res = await fetch(`/api/${tenantSlug}/business/companies/${company._id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleAddEmployee(companyId: string) {
    ...
    try {
      const res = await fetch(`/api/${tenantSlug}/business/companies/${companyId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-6">
        ...
        <form onSubmit={handleCreate} className="p-8 bg-card border-2 border-border/60 rounded-[2.5rem] shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground/50 font-medium mt-1">Este mail puede abrir pedidos grupales y tiene acceso a pago diferido</p>
            </div>
            <div>
          ...
          <div>
            ...
            <input value={form.paymentTerms} onChange={e => setForm(p => ({ ...p, paymentTerms: e.target.value }))} className={inputCls} placeholder="Ej: Pago a 30 días desde la fecha de factura" />
          </div>
          ...
          <div>
            ...
            />
            <p className="text-[10px] text-muted-foreground/50 font-medium mt-1">Los empleados acceden al menú Business con su email. Pagan siempre con MP.</p>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground/50 mt-1">Agregá una empresa para que sus empleados accedan al menú Business</p>
        </div>
      ) : (
