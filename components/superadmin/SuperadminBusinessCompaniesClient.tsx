...
import {
  Building2, Plus, X, Check, Loader2, Search, Trash2, ToggleLeft, ToggleRight, Store,
} from 'lucide-react'
          ...
          {filtered.map(company => (
            <div key={company._id} className="p-6 bg-card border-2 border-border/60 rounded-[2rem] shadow-lg space-y-4">
              ...
              {company.employeeEmails.length > 0 && (
                <div>
                  <p className="text-[10px] uppercase font-black tracking-widest text-muted-foreground/50 mb-2">Empleados Habilitados</p>
                  <div className="flex flex-wrap gap-2">
                    {company.employeeEmails.map(email => (
                      <div key={email} className="px-3 py-1.5 rounded-full bg-muted/40 border border-border/40">
